...
- If the task changes frontend feature boundaries, state ownership, persistence shape, or feature-internal layer responsibilities, read [`../docs/architecture.md`](../docs/architecture.md) and [`../docs/layering.md`](../docs/layering.md).
...
- For order-entry route state and stepper behavior, read [`src/app/features/order-entry/order-entry.store.ts`](src/app/features/order-entry/order-entry.store.ts), [`src/app/features/order-entry/order-entry.ts`](src/app/features/order-entry/order-entry.ts), and [`src/app/features/order-entry/order-entry.store.spec.ts`](src/app/features/order-entry/order-entry.store.spec.ts).
- For the durable room list, read [`src/app/features/rooms/rooms.store.ts`](src/app/features/rooms/rooms.store.ts), [`src/app/features/rooms/rooms.repository.ts`](src/app/features/rooms/rooms.repository.ts), and [`src/app/features/rooms/rooms.store.spec.ts`](src/app/features/rooms/rooms.store.spec.ts).
- For the historical drink-tally guest-flow presentation, read [`src/app/features/drink-tally/drink-tally.ts`](src/app/features/drink-tally/drink-tally.ts), [`src/app/features/drink-tally/drink-tally.html`](src/app/features/drink-tally/drink-tally.html), and [`src/app/features/drink-tally/drink-tally.scss`](src/app/features/drink-tally/drink-tally.scss).
...
- For feature-level UI tests, read [`src/app/features/order-entry/order-entry.spec.ts`](src/app/features/order-entry/order-entry.spec.ts) for route integration behavior and [`src/app/features/drink-tally/drink-tally.spec.ts`](src/app/features/drink-tally/drink-tally.spec.ts) for the presentational screen contract.
