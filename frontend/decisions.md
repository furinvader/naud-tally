...
- Consequence: [`eslint.config.js`](eslint.config.js) is the source of truth for the automated boundary rule under [`src/app/`](src/app/)
- Consequence: the frontend workspace uses Angular ESLint through the [`lint`](angular.json) target in [`angular.json`](angular.json) and the [`lint`](package.json) script in [`package.json`](package.json)
