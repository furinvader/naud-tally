...
- Keep the current design brief in a repo-native markdown file such as [`docs/penpot-example-drink-tally.md`](penpot-example-drink-tally.md).
- Keep browser-viewable design exports in [`../design/`](../design/), with SVG preferred and PNG preview files alongside when helpful.
...
5. After a meaningful design change, export a committed design artifact to [`../design/`](../design/) and save the artifact path plus any implementation notes back into the brief.
...
- Prefer SVG as the repo-tracked design artifact because it is readable by Codex and opens directly in a browser.
- Keep a PNG export next to the SVG when you want a quick preview in GitHub or markdown docs.
- Keep the markdown brief as the place for intent, open questions, and implementation notes.
- Treat the committed artifact as the default handoff for the repo and the optional Penpot share link as a convenience layer for live review.
...
- preferred repo SVG export path
- repo PNG preview path, if present
...
[`docs/penpot-example-drink-tally.md`](penpot-example-drink-tally.md) is the first example of that format.

## Recommended Codex Prompt

Use a prompt shaped like this once the Penpot file and plugin are live:

> Use Penpot to update the active design file using [`docs/penpot-example-drink-tally.md`](./penpot-example-drink-tally.md) as the source of truth. Create the described smoke-test layout, then summarize what you created and any gaps between the brief and Penpot.
