...
It complements the product scope in [`docs/product.md`](docs/product.md), the workflow guidance in [`docs/agentic-workflow.md`](docs/agentic-workflow.md), the repository decisions in [`docs/decisions.md`](docs/decisions.md), the feature-internal layer rules in [`docs/architecture/layering.md`](docs/architecture/layering.md), and the frontend-local conventions in [`frontend/decisions.md`](frontend/decisions.md).
...
We want a [modular monolith](docs/glossary.md#modular-monolith):
...
- `order-entry`: the route-level [composition root](docs/glossary.md#composition-root) for the current main pilot screen
- `rooms`: the fixed room list that the host configures and the order-entry route consumes
- `guest-tabs`: open [guest tabs](docs/glossary.md#guest-tab), guest identity rules, and tab lifecycle rules
- `catalog`: product catalog and price-management rules
- `billing-history`: billed-tab creation and recent billed history
- `sync-recovery`: [sync status](docs/glossary.md#sync-status), local [outbox](docs/glossary.md#outbox) or operation log, and remote recovery integration
...
- keep feature-root `index.ts` files as the feature [public API](docs/glossary.md#public-api), not as internal layer shortcuts
...
The general layer responsibilities and dependency rules live in [`docs/architecture/layering.md`](docs/architecture/layering.md). Concrete file naming examples should live in the nearest area-specific docs such as [`frontend/README.md`](frontend/README.md).

## Ownership Rules

### Route Composition

The route feature for the current order entry screen should be the [composition root](docs/glossary.md#composition-root).
...
Each durable [capability feature](docs/glossary.md#capability-feature) should own:

- its domain models
- its [domain rules](docs/glossary.md#domain-rule)
- its [persistent business state](docs/glossary.md#persistent-business-state)
- its adapters and serialization rules
- its focused tests

Each capability should expose a small [public API](docs/glossary.md#public-api), usually through a facade or clearly named public file.
...
- capability features should not [deep-import](docs/glossary.md#deep-import) each other's internal files
...
- [domain rules](docs/glossary.md#domain-rule) should not depend on browser storage or remote APIs directly

If one feature needs another feature's behavior, add or refine the other feature's [public API](docs/glossary.md#public-api) instead of importing its internal store or helper file directly.
...
[Persistent business state](docs/glossary.md#persistent-business-state) should live with the capability that owns the business concept.
...
[Transient route state](docs/glossary.md#transient-route-state) should live with the route composition feature.
...
Capability features should read and write through [repositories](docs/glossary.md#repository) or equivalent [adapters](docs/glossary.md#adapter).
...
1. validate the user action against [domain rules](docs/glossary.md#domain-rule)
2. update local capability state
3. persist locally through the capability [repository](docs/glossary.md#repository)
4. record sync metadata or queue a sync operation when that architecture is introduced

### Sync and Recovery Boundary

The app will eventually need a local [outbox](docs/glossary.md#outbox) or operation-log-style boundary for remote recovery and sync.
...
- start architecture-changing tasks from [`ARCHITECTURE.md`](ARCHITECTURE.md), [`docs/architecture/layering.md`](docs/architecture/layering.md), [`docs/glossary.md`](docs/glossary.md), and [`frontend/README.md`](frontend/README.md)
- prefer one capability-focused task at a time
- add a local `README.md` when a feature area becomes large enough that its public API is not obvious
...
1. [T-024 Create the Order Entry Composition Root](docs/tasks/done/T-024.md)
2. [T-025 Separate Order Entry UI State From Persistent Business State](docs/tasks/done/T-025.md)
3. [T-026 Split Tally Logic Into Guest Tabs, Catalog, and Billing History Modules](docs/tasks/done/T-026.md)
4. [T-027 Introduce Repository Adapters for Local Persistence](docs/tasks/done/T-027.md)
5. [T-028 Expose Feature Public APIs and Remove Cross-Feature Internal Imports](docs/tasks/done/T-028.md)
6. [T-029 Add Frontend Import Boundary Checks](docs/tasks/done/T-029.md)
